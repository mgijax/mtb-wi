...
function showResults(){
	...
	html += "<th>Treatment Type<span class='type'>Agents</span></th>";
	html += "<th onclick='colSort(2)'>Strain Name <img src='"+getSortImage(2)+"'><span class='type'>Strain Types</span></th>";
html += "<th onclick='colSort(3)'><dl class=\"tip\">Reported Frequency (sex)<div><h4>Frequency Color Legend</h4>" + legend + "</div></div> <img src='"+getSortImage(3)+"'>";
	html += "</th><th>Metastasizes To</th>";
	...
	for(var i=0; i < rsp.response.docs.length; i++){
		...
		html += "<a href='${pageContext.request.contextPath}/tumorSummary.do?tumorFrequencyKeys="+tfKeys+"' target='_blank'>";
...
function minFCCB(){
	minFC = document.getElementById("min-fc").checked;
	updatePageFromZero();
}
function asCSVCB(){
	asCSV = document.getElementById("as-csv").checked;
	if(asCSV){
		resLimit = 1000;
	}else{
		resLimit = 25;
...
function mutantCheck(name,search){
	...
	nMutant =document.getElementById("non-mutant-strains");
	if(name=="a"){
		all.checked = true;
