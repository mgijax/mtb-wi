...
<jax:mmhcpage title="Patient Derived Xenograft Copy Number Variation" help="userHelp.jsp#pdxcnv">
				...
				if(rankData.getNumberOfRows()>0){
					...
					rankBarChart.draw(rankView, options);
                        ...
                        function toggleDisplay(){
                            ...
                            rank(showDiagnosis);
                            if(showDiagnosis){
                                document.getElementById("toggleButton").value="Order by Log Ratio Ploidy"
                            }else{
		...
		<section id="detail">
			<c:choose>
				...
				<c:otherwise>
					...
					<h3>Log Ratio Ploidy ${gene2}</h3>
					<p>${message}</p>
					<p>Click on a LRP bar to go to the model's details page.</p>
